...
def list_sessions_for_obs():
    """Lightweight session list for the obs session browser (local-only, no auth)."""
    try:
        conn = _get_db()
        rows = conn.execute(
...
def get_session_obs_events(session_id):
    """Return obs events for a session. Checks obs_events table first, then reconstructs from llm_calls + session_steps."""
    try:
        conn = _get_db()
        # Check for stored obs events first
        stored = conn.execute(
        ...
        # Sort by timestamp and compute elapsed_s
