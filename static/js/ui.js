...
// ═══════════════════════════════════════════════════════════════════════════
// ADSENSE GATING — non-intrusive, respectful ad serving
// Rules:
//   1. Never show ads to brand-new visitors (need at least 1 prior visit)
//   2. User must have spent 5+ minutes active on the site this session
//   3. At least 5 minutes between ad impressions
//   4. Show ad every 10 page/view navigations
// ═══════════════════════════════════════════════════════════════════════════

const _AD_CONFIG = {
  minVisits: 1,           // prior visits before any ad
  minActiveMinutes: 5,    // minutes of active time this session
  minIntervalMinutes: 5,  // minutes between ad swaps
  viewsPerAd: 10,         // page views between ads
};

(function initAdGating() {
  // Track visits
  let visits = 0;
  try { visits = parseInt(localStorage.getItem('arc_visit_count') || '0', 10); } catch {}
  const isNewVisitor = visits < _AD_CONFIG.minVisits;
  try { localStorage.setItem('arc_visit_count', String(visits + 1)); } catch {}

  if (isNewVisitor) return; // Rule 1: no ads for new visitors

  let _adActiveSeconds = 0;
  let _adViewCount = 0;
  let _adLastShown = 0;
  let _adActive = true; // track if tab is focused

  // Track active time (only count when tab is visible)
  document.addEventListener('visibilitychange', () => { _adActive = !document.hidden; });
  setInterval(() => { if (_adActive) _adActiveSeconds++; }, 1000);

  // Track view navigations
  const origShowAppView = window.showAppView;
  if (origShowAppView) {
    window.showAppView = function() {
      _adViewCount++;
      origShowAppView.apply(this, arguments);
      _checkAdEligibility();
    };
  }

  function _checkAdEligibility() {
    // Rule 2: 5+ min active
    if (_adActiveSeconds < _AD_CONFIG.minActiveMinutes * 60) return;
    // Rule 3: 5+ min since last ad
    if (Date.now() - _adLastShown < _AD_CONFIG.minIntervalMinutes * 60 * 1000) return;
    // Rule 4: every N views
    if (_adViewCount % _AD_CONFIG.viewsPerAd !== 0) return;

    _showAdsenseSlot();
  }

  function _showAdsenseSlot() {
    const bmcSlot = document.getElementById('bmcSlot');
    const adsenseSlot = document.getElementById('adsenseSlot');
    if (!bmcSlot || !adsenseSlot) return;

    // Only inject AdSense script once
    if (!window._adsenseLoaded && window.ARC_ADSENSE_CLIENT) {
      const s = document.createElement('script');
      s.async = true;
      s.crossOrigin = 'anonymous';
      s.src = 'https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=' + window.ARC_ADSENSE_CLIENT;
      document.head.appendChild(s);
      window._adsenseLoaded = true;
    }

    // Swap: show AdSense, hide BMC
    bmcSlot.style.display = 'none';
    adsenseSlot.style.display = '';
    adsenseSlot.innerHTML = '';

    if (window.ARC_ADSENSE_CLIENT && window.ARC_ADSENSE_SLOT) {
      const ins = document.createElement('ins');
      ins.className = 'adsbygoogle';
      ins.style.display = 'block';
      ins.setAttribute('data-ad-client', window.ARC_ADSENSE_CLIENT);
      ins.setAttribute('data-ad-slot', window.ARC_ADSENSE_SLOT);
      ins.setAttribute('data-ad-format', 'auto');
      ins.setAttribute('data-full-width-responsive', 'true');
      adsenseSlot.appendChild(ins);
      try { (window.adsbygoogle = window.adsbygoogle || []).push({}); } catch {}
    } else {
      // Fallback: no AdSense configured, show a subtle self-promo
      adsenseSlot.innerHTML = '<div style="font-size:11px;color:var(--text-dim);padding:8px;text-align:center;">Ad slot available — <a href="#feedback" onclick="event.preventDefault();showAppView(\'feedback\')" style="color:var(--accent);">give us feedback</a></div>';
    }

    _adLastShown = Date.now();

    // Swap back to BMC after 30 seconds
    setTimeout(() => {
      bmcSlot.style.display = '';
      adsenseSlot.style.display = 'none';
    }, 30000);
  }

  // Also check periodically (every 60s) in case user stays on one view
  setInterval(_checkAdEligibility, 60000);
})();
