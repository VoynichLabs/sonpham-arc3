...
def get_move(state):
    ...
    # Cache wall set once (walls don't change mid-game)
    if 'wall_set' not in mem:
        mem['wall_set'] = [tuple(w) for w in state['walls']]
    # Track enemy positions for prediction
    if 'enemy_history' not in mem:
        mem['enemy_history'] = []
    if state['enemy_snake']:
        mem['enemy_history'].append(tuple(state['enemy_snake'][0]))
...
- Use it for: wall graph caches, enemy movement models, corridor maps, BFS result caching, etc.
