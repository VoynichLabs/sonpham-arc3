...
def get_move(state):
    ...
    if 'enemy_positions' not in mem:
        mem['enemy_positions'] = []
    mem['enemy_positions'].append(tuple(state['enemy_snake'][0]) if state['enemy_snake'] else None)

    # Use accumulated data for smarter decisions
    if len(mem['enemy_positions']) > 5:
        # Predict enemy movement from history...
        pass
...
- Use it for: enemy movement models, flood-fill caches, danger zone maps, strategy FSMs, etc.
