...
def get_move(state):
    ...
    # Track ally and enemy positions for coordination
    if 'ally_heads' not in mem:
        mem['ally_heads'] = []
    if state['ally_snake']:
        mem['ally_heads'].append(tuple(state['ally_snake'][0]))

    # Track enemy movement patterns
    if 'enemy_history' not in mem:
        mem['enemy_history'] = [[], []]
    for i, e in enumerate(state['enemies']):
        if e['alive'] and e['body']:
            mem['enemy_history'][i].append(tuple(e['body'][0]))

    # Detect when ally dies → switch to survival mode
    if 'mode' not in mem:
        mem['mode'] = 'team'
    if not state['ally_snake']:
        mem['mode'] = 'solo_survival'
...
- Use it for: team coordination state, enemy prediction models, zone assignment tracking, ally health monitoring, etc.
